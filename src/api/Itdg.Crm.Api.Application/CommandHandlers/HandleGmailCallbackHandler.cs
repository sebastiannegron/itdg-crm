...
public class HandleGmailCallbackHandler : ICommandHandler<HandleGmailCallback>
{
    ...
    public HandleGmailCallbackHandler(
        ...
        ILogger<HandleGmailCallbackHandler> logger)
    {
    ...
    public async Task HandleAsync(HandleGmailCallback command, string language, Guid correlationId, CancellationToken cancellationToken)
    {
        using Activity? activity = DiagnosticsConfig.ActivitySource.StartActivity("Handle Gmail OAuth Callback");
        activity?.SetTag("CorrelationId", correlationId);

        _logger.LogInformation("Processing Gmail OAuth callback | CorrelationId: {CorrelationId}", correlationId);
        ...
        var existingToken = await _tokenRepository.GetByUserIdAndProviderAsync(user.Id, "Gmail", cancellationToken);
        ...
        if (existingToken is not null)
        {
            ...
            _logger.LogInformation("Updated Gmail tokens for user {UserId} | CorrelationId: {CorrelationId}", user.Id, correlationId);
        }
        else
        {
            ...
            _logger.LogInformation("Stored Gmail tokens for user {UserId} | CorrelationId: {CorrelationId}", user.Id, correlationId);
        }
    }
}
